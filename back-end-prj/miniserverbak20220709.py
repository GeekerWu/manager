...
def save_voice():
    ...
    data.append((dictobj['speechtext'],'./voiceasset/'+dictobj['speechtext']+'.wav',dictobj['mean'],dictobj['languagetype'],dictobj['steps']))
    print('sql data:',data)
    #insert data:
    conn = pymysql.connect(
        host = 'localhost',
    ...
    sql = "insert into voice_asset(text,asset_path,mean,language_type,steps) values(%s,%s,%s,%s,%s);"
    # data = [
    #     ('july', '147'),
    #     ('june', '258'),
    #     ('marin', '369')
...
def search_voice():
    ...
    for item in res:
        ...
        if data=='':
            data = data + '[' + json.dumps(
                {"asset_id": item[0], "text": item[1], "asset_path": item[2], "mean": item[3], "language_type": item[4], "steps": item[5]})
        else:
            data=data+','+json.dumps({"asset_id": item[0], "text": item[1], "asset_path": item[2], "mean": item[3], "language_type": item[4], "steps": item[5]})
        #print('json item:',jsonitem)
    data=data+']'
    # print(dictobj['a'])
    # 关闭连接
