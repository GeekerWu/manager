...
def searchtable():
    ...
    for item in res:
        ...
        if data=='':
            data = data + '[' + json.dumps(
                {"animation_name": item[0], "channel": item[1], "channel_name": item[2], "channel_value": item[3], "time_stamp": item[4]})
        else:
            data=data+','+json.dumps({"animation_name": item[0], "channel": item[1], "channel_name": item[2], "channel_value": item[3], "time_stamp": item[4]})
        #print('json item:',jsonitem)
    data=data+']'
    # print(dictobj['a'])
    # 关闭连接
