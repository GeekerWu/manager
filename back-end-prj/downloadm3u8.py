...
import ssl
import random
path='C:/Users/GeekerWu/Desktop/manager/back-end-prj/tsvideo/'
'''
...
#https://huangzhongtv.com/vodplay/49886-1-1.html
m3u8url='https://a.ak-kk.com/20220807/j1NuwwDx/hls/index.m3u8'
#无网址list 需要添加 有网址置空
resoucesite=''
#key 中 resource对应地址串 替换成本地
replacestr='https://vo1.123188kk.com/20220807/j1NuwwDx/hls/'
#目标文件名
filmname='异世界迷宫里的后宫生活03'

#https://huangzhongtv.com/vodplay/49886-1-1.html
m3u8url='https://a.ak-kk.com/20220902/bNDX6RQC/hls/index.m3u8'
#无网址list 需要添加 有网址置空
resoucesite=''
#key 中 resource对应地址串 替换成本地
replacestr='https://vo1.123188kk.com/20220902/bNDX6RQC/hls/'
#目标文件名
filmname='异世界迷宫里的后宫生活08'
...
#https://huangzhongtv.com/vodplay/49886-1-1.html
m3u8url='https://a.ak-kk.com/20221129/dfqmbriA/hls/index.m3u8'
#无网址list 需要添加 有网址置空
resoucesite=''
#key 中 resource对应地址串 替换成本地
replacestr='https://vo1.123188kk.com/20221129/dfqmbriA/hls/'

#目标文件名
filmname='雌性友人01'

#https://huangzhongtv.com/vodplay/49886-1-1.html
m3u8url='https://a.ak-kk.com/20230202/A1RxgK1x/hls/index.m3u8'
#无网址list 需要添加 有网址置空
resoucesite=''
#key 中 resource对应地址串 替换成本地
replacestr='https://vo1.123188kk.com/20230202/A1RxgK1x/hls/'
...
'''
https://askzybf.com/20230325/uDfv9qop/2000kb/hls/pCBquRHN.ts
https://askzybf.com/20230325/uDfv9qop/2000kb/hls/gDllIu72.ts
'''
#https://www.91zx13.xyz/vodplay/693270s1n1/
#askzybf.com/20220930/PAp6Daf7/2000kb/hls/key.key
m3u8url='https://askzybf.com/20220930/PAp6Daf7/2000kb/hls/index.m3u8'
#无网址list 需要添加 有网址置空
resoucesite='https://askzybf.com'
#key 中 resource对应地址串 替换成本地
replacestr='/20220930/PAp6Daf7/2000kb/hls/'

#目标文件名（不能有空格）
filmname='FirstAssembly'
...
def getM3u8():
    ...
    #文件存成index.m3u8
    with open(f'tsvideo\\my.m3u8', 'w', encoding='utf-8') as fp:
        fp.write(con)

# 通过m3u8找到ts文件的路径
def getUrlOfTs():
    #with open(f'tsvideo\\my.m3u8', 'r', encoding='utf-8') as fp:
    ##support continue
    with open(f'tsvideo\\Urllist.txt', 'r', encoding='utf-8') as fp:
        con = fp.read()
        # print(con)
        '''
        ...
        for i in tmps:
            ...
            if i.group().find('key.key') > 0:
                ...
                filedata += f"{tsUrl}\n"
        print(urls)
        with open(f'tsvideo\\Urllist.txt',"w",encoding="utf-8") as f:
            f.write(filedata)
        '''
...
def alter(file,old_str,path):
    ...
    """
    file_data = ""
    with open(f'tsvideo\\{file}', "r", encoding="utf-8") as f:
        for line in f:
            if old_str in line:
            ...
            file_data += line
    with open(f'tsvideo\\index.m3u8',"w",encoding="utf-8") as f:
        f.write(file_data)
...
def alter2():
    ...
    """
    file_data = ""
    with open(f'tsvideo\\my.m3u8', "r", encoding="utf-8") as f:
        for line in f:
            if 'key.key' in line:
                line = line.replace('key.key', 'key.m3u8')
...
def loadTs(urls):
    ...
    for url in urls:
        ...
        '''
        headers = {
            'authority': 'askzybf.com',
            'method': 'GET',
            'path': '/20230325/uDfv9qop/2000kb/hls/gDllIu72.ts',
            'scheme': 'https',
            'accept': '*/*',
            'accept-encoding': 'gzip, deflate, br',
            'accept-language': 'zh-CN,zh;q=0.9',
            'Connection': 'keep-alive',
            'host': 'askzybf.com',
            'origin': 'https://sljxsl.com',
            'referer': 'https://sljxsl.com/',
        ...
        if url.find('key.key') > 0:
            ...
            #title = str(num) + '.ts'
        with open(f'tsvideo\\{title}', 'wb') as fp:
            fp.write(con)
        print(str(num) + title + '下载完成~~~')
        file_data = ""
        with open(f'tsvideo\\Urllist.txt', "r", encoding="utf-8") as f:
            for line in f:
                if f'{title}' in line:
...
def hebingTs(endNum):
    for i in range(1, endNum + 1):
        with open(filmname+'.mp4', 'ab+') as fp:
            fp.write(open(f'.\\tsvideo\\{i}.ts', 'rb').read())
    print('合并完成！')
'''
def join_video():
    """
    ...
    #command1 = 'cd C:/Users/GeekerWu/Desktop/manager/back-end-prj/tsvideo'
...
def join_video2():
    ...
    filepath = "C:/Users/GeekerWu/Desktop/manager/back-end-prj/tsvideo/"
    ...
    with open(f'.\\tsvideo\\file_list.txt',"w", encoding="utf-8") as f:
        f.write(file_data)
    '''


    file_data = ""
    with open(f'tsvideo\\index.m3u8', "r", encoding="utf-8") as f:
        for line in f:
            if line.find('.ts') > 0:
                sourcestr=line.replace('\n', '')
                file_data +=f"file '{sourcestr}'\n"
    with open(f'tsvideo\\file_list.txt', "w", encoding="utf-8") as f:
        f.write(file_data)
...
def main():
    ...
    # 打开my.m3u8 编辑相对路径并将（replacestr）http或者其他地址替换为本地地址（path），另存为 index.m3u8 合并生成mp4时的索引文件
    ...
    join_video2()
    '''
    #根据index.m3u8 和key.m3u8 合并 ts 为mp4
    join_video()
