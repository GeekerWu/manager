...
def search_event():
    ...
    for item in res:
        ...
        if data=='':
            data = data + '[' + json.dumps(
                {"asset_id": item[0], "event_name": item[1], "event_condition": item[2], "animation": item[3], "voice": item[4], "voice_start": item[5]})
        else:
            data=data+','+json.dumps({"asset_id": item[0], "event_name": item[1], "event_condition": item[2], "animation": item[3], "voice": item[4], "voice_start": item[5]})
        #print('json item:',jsonitem)
    data=data+']'
    # print(dictobj['a'])
    # 关闭连接
...
def play_event():
    ...
    cur.execute("select voice ,voice_start from event_asset where event_name ='"+eventname+"' order by voice_start")# 查询数据   
...
def save_voice():
    ...
    data.append((dictobj['speechtext'],'./voiceasset/'+dictobj['speechtext']+'.wav',dictobj['mean'],dictobj['languagetype'],dictobj['steps'],str(round(duration,1))))
    print('sql data:',data)
    #insert data:
    conn = pymysql.connect(
        host = 'localhost',
...
def get_voice():
    ...
    url = 'https://fanyi.baidu.com/gettts?lan='+dictobj['languagetype']+'&text='+dictobj['speechtext']+'&spd=1&source=web'
    r = requests.get(url) 
...
def search_voice():
    ...
    for item in res:
        ...
        if data=='':
            data = data + '[' + json.dumps(
                {"asset_id": item[0], "text": item[1], "asset_path": item[2], "mean": item[3], "language_type": item[4], "steps": item[5], "duration": item[6]})
        else:
            data=data+','+json.dumps({"asset_id": item[0], "text": item[1], "asset_path": item[2], "mean": item[3], "language_type": item[4], "duration": item[6]})
        #print('json item:',jsonitem)
    data=data+']'
    # print(dictobj['a'])
    # 关闭连接
...
def searchtable():
    ...
    for item in res:
        ...
        if data=='':
            data = data + '[' + json.dumps(
                {"animation_name": item[0], "channel": item[1], "channel_name": item[2], "channel_value": item[3], "time_stamp": item[4], "timeline": item[5]})
        else:
            data=data+','+json.dumps({"animation_name": item[0], "channel": item[1], "channel_name": item[2], "channel_value": item[3], "time_stamp": item[4], "timeline": item[5]})
        #print('json item:',jsonitem)
    data=data+']'
    # print(dictobj['a'])
    # 关闭连接
...
def update_anima():
    ...
    for item in dictobj:
        ...
        data.append((item['animaname'],item['channel'] ,item['channel_name'],item['currval'],item['timestamp'],item['timeline']))
...
def update_event():
    ...
    for item in dictobj:
        #print('item:',item)
        # print('item:', item['channel'])
        data.append((item['event_name'],item['event_condition'] ,item['animation'],item['voice'],item['voice_start']))
        # data.append('('+item['channel']+','+item['min']+','+item['max']+','+item['init']+')')
    # dictobj = json.loads(data)
    # print(dictobj['a'])
    #print('sql data:',data)
